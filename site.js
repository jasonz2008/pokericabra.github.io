...
function indiePage(){return `<section class="project-hero indie-hero indie-intro"><h1>${t('Ongoing Project Development','開發中 · 項目前瞻')}</h1><p class="hero-sub">${t('Project management system development','項目管理系統開發')}</p><div class="hero-meta">${t('Built from 0 to 1 · Product direction, AI collaboration & implementation','由 0 至 1 建立產品 · 產品方向、AI 協作及實作')}</div>${ProjectProcess.teaser(true)}</section>${IndieEvolution.render()}${ProjectProcess.render(true)}${indieGallery()}`;}
